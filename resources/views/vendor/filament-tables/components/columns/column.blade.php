...
<div @if (filled($tooltip)) x-data="{}"
    ...
    @if (($url || ($recordUrl && $action === null)) && !$isClickDisabled)
        <a {{ \Filament\Support\generate_href_html($url ?: $recordUrl, $url ? $shouldOpenUrlInNewTab : $shouldOpenRecordUrlInNewTab) }}
            class="{{ $columnClasses }}">
            {{ $slot }}
        </a>
    @elseif (($action || $recordAction) && !$isClickDisabled)
        ...
        <button type="button" wire:click="{{ $wireClickAction }}" wire:loading.attr="disabled"
            wire:target="{{ $wireClickAction }}" class="{{ $columnClasses }}">
            {{ $slot }}
        </button>
    @else
